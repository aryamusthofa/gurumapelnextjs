...
import SiswaProfileModal from "./SiswaProfileModal";
...
export default function SiswaLayout({ children }) {
    ...
    const menuItems = [
        { name: "My Dashboard", href: "/siswa", icon: "M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" },
    ];
            ...
            >
                <div className="flex flex-col h-full overflow-hidden">
                    <div className="p-8 pb-4 shrink-0">
                        <div className="flex items-center gap-4 group cursor-pointer" onClick={() => setSidebarOpen(false)}>
                            ...
                            <div className="flex flex-col">
                                <span className="text-xl font-black tracking-tighter uppercase italic">Siswa <span className="text-blue-600">Portal</span></span>
                    ...
                    <nav className="flex-1 px-6 py-8 space-y-2 overflow-y-auto no-scrollbar">
                        <p className="px-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.3em] mb-4">Navigasi Siswa</p>
                        {menuItems.map((item) => {
                ...
                <header className="sticky top-0 z-40 bg-white/40 dark:bg-slate-950/40 backdrop-blur-3xl px-8 h-24 border-b border-white/20 dark:border-white/5 flex items-center justify-between">
                    <div className="flex items-center gap-8">
                        ...
                        <div className="flex flex-col">
                            ...
                            <h2 className="text-lg sm:text-xl font-black tracking-tighter uppercase italic" title={user?.nama}>
                                {user?.nama?.length > 7 ? user.nama.substring(0, 7) + "..." : (user?.nama || "Siswa")}
                            </h2>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        <button
                            ...
                            className="w-12 h-12 glass-effect rounded-2xl flex items-center justify-center transition-all hover:scale-105 active:scale-95 shadow-lg group"
                        >
                            {theme === 'dark' ? (
                        ...
                        >
                            ...
                            <div className="hidden md:block text-right">
                                <p className="text-[10px] font-black text-blue-600 leading-none uppercase tracking-widest mb-1">Student</p>
                                <p className="text-xs font-black uppercase italic" title={user?.nama}>
                                    {user?.nama?.length > 10 ? user.nama.substring(0, 10) + "..." : user?.nama}
                                </p>
                            </div>
                        </div>
                    </div>
