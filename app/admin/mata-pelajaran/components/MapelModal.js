...
export default function MapelModal({ isOpen, onClose, onSuccess, editData }) {
    const [form, setForm] = useState({ nama_mapel: "", kode_mapel: "" });
    ...
    useEffect(() => {
        if (isOpen && editData) {
            setForm({ nama_mapel: editData.nama_mapel || "", kode_mapel: editData.kode_mapel || "" });
        } else if (isOpen) {
            setForm({ nama_mapel: "", kode_mapel: "" });
        }
        setError("");
    }, [isOpen, editData]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!form.nama_mapel || !form.kode_mapel) { setError("Nama mapel dan kode mapel wajib diisi"); return; }
        setSubmitting(true);
        try {
            const url = isEdit ? `/api/admin/mapel/${editData.id}` : "/api/admin/mapel";
            const res = await fetch(url, {
                method: isEdit ? "PUT" : "POST", credentials: "include",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ nama_mapel: form.nama_mapel, kode_mapel: form.kode_mapel }),
            });
            const data = await res.json();
            if (!res.ok) throw new Error(data.message || "Gagal memproses data");
            handleClose();
            onSuccess(isEdit ? "Mata pelajaran berhasil diperbarui!" : "Mata pelajaran baru berhasil ditambahkan!");
            ...
            <div className={`relative w-full max-w-lg bg-slate-50 dark:bg-slate-900 rounded-[2.5rem] shadow-2xl border border-slate-200 dark:border-white/10 p-8 transition-all duration-300 ease-out ${visible ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-4'}`}>
                ...
                <div className="mb-8">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <h2 className="text-xl font-black tracking-tight text-slate-800 dark:text-white">{isEdit ? "Edit Mapel" : "Registrasi Mapel"}</h2>
                    </div>
                    <p className="text-sm text-slate-400 font-bold ml-[52px]">{isEdit ? "Perbarui data mata pelajaran." : "Daftarkan mata pelajaran baru."}</p>
                </div>

                <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Nama Pelajaran</label>
                        <input type="text" value={form.nama_mapel} onChange={(e) => setForm({ ...form, nama_mapel: e.target.value })} className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-2xl p-4 font-bold text-slate-700 dark:text-white outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all" placeholder="e.g. Matematika Terapan" autoFocus />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Kode Unik</label>
                        <input type="text" value={form.kode_mapel} onChange={(e) => setForm({ ...form, kode_mapel: e.target.value.toUpperCase() })} className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-2xl p-4 font-bold text-slate-700 dark:text-white outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all uppercase" placeholder="e.g. MTK-01" />
                    </div>
