...
export default function GuruModal({ isOpen, onClose, onSuccess, editData }) {
    const [form, setForm] = useState({ nama: "", email: "", nis_nip: "" });
    ...
    useEffect(() => {
        if (isOpen && editData) {
            setForm({ nama: editData.nama || "", email: editData.email || "", nis_nip: editData.nis_nip || "" });
        } else if (isOpen) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setError("");
        if (!form.nama || !form.email || !form.nis_nip) { setError("Semua field wajib diisi"); return; }
        setSubmitting(true);
        try {
            const url = isEdit ? `/api/admin/guru/${editData.id}` : "/api/admin/guru";
            ...
            handleClose();
            onSuccess(isEdit ? "Data guru berhasil diperbarui!" : "Guru baru berhasil ditambahkan!");
            ...
            <div className={`relative w-full max-w-lg bg-slate-50 dark:bg-slate-900 rounded-[2.5rem] shadow-2xl border border-slate-200 dark:border-white/10 p-8 transition-all duration-300 ease-out ${visible ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-4'}`}>
                ...
                <div className="mb-8">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <h2 className="text-xl font-black tracking-tight text-slate-800 dark:text-white">{isEdit ? "Edit Guru" : "Registrasi Guru"}</h2>
                    </div>
                    <p className="text-sm text-slate-400 font-bold ml-[52px]">{isEdit ? "Perbarui data tenaga pendidik." : "Daftarkan guru baru ke sistem."}</p>
                </div>

                <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Nama Lengkap</label>
                        <input type="text" value={form.nama} onChange={(e) => setForm({ ...form, nama: e.target.value })} className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-2xl p-4 font-bold text-slate-700 dark:text-white outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all" placeholder="e.g. Dr. Budi Santoso" autoFocus />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Email Akademik</label>
                        <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value.toLowerCase() })} className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-2xl p-4 font-bold text-slate-700 dark:text-white outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all" placeholder="e.g. budi@school.id" />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">NIP</label>
                        <input type="text" value={form.nis_nip} onChange={(e) => setForm({ ...form, nis_nip: e.target.value.replace(/[^0-9]/g, '') })} className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-2xl p-4 font-bold text-slate-700 dark:text-white outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all" placeholder="e.g. 19800101..." />
                    </div>
                    ...
                    <div className="flex gap-3 pt-4">
                        <button type="submit" disabled={submitting} className={`flex-1 py-4 rounded-2xl font-black text-white shadow-xl transition-all active:scale-95 disabled:opacity-60 ${isEdit ? 'bg-gradient-to-r from-green-600 to-green-500 shadow-green-500/20' : 'bg-gradient-to-r from-blue-600 to-blue-500 shadow-blue-500/20'}`}>
                            {submitting ? "Memproses..." : isEdit ? "PERBARUI" : "TAMBAHKAN"}
                        </button>
